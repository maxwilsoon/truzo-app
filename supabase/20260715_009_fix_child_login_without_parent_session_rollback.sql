...
--
-- WARNING: After rollback, child login will only work when a parent Supabase
-- Auth session is active. Cold-restart child login will silently fail again.

-- Restore login_child with AND parent_id = auth.uid() (migration 008 state)
...
-- Restore biometric_login_child with AND parent_id = auth.uid() (migration 006 state)
